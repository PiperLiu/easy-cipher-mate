...
function createEncryption(algorithm: string = 'aes-gcm'): [IEncryptionAlgorithm<any>, (password: string, encoding?: TextEncoding) => IEncryptionAlgorithmConfig] {
  switch (algorithm.toLowerCase()) {
    case 'chacha20-poly1305':
    case 'chacha20':
      return [
        new ChaCha20Poly1305Encryption(),
        (password: string, encoding?: TextEncoding) => new ChaCha20Poly1305EncryptionConfigFromEnv(password, undefined, undefined, encoding)
      ];
    case 'aes-gcm':
    case 'aes':
    default:
      return [
        ...
        (password: string, encoding?: TextEncoding) => new AESGCMEncryptionConfigFromEnv(password, undefined, undefined, encoding)
      ];
  }
}
...
const encryptTextFileCommand = program
  ...
  .action(async (options) => {
    try {
      const [encryption, configFactory] = createEncryption(options.algorithm);
      const config = configFactory(options.password, options.encoding as TextEncoding);
      const service = new EncryptionService(encryption, config);
      ...
      const encryptedLines = await Promise.all(
        lines.map(async line => {
          ...
          const result = await service.encryptText(line, options.encoding as TextEncoding);
      ...
      const decryptedLines = await Promise.all(
        lines.map(async (line, index) => {
          ...
          try {
            ...
            return await service.decryptText(buffer, options.encoding as TextEncoding);
          } catch (e) {
